...
extern MessageBufferHandle_t xMessageBufferBle;
extern MessageBufferHandle_t xMessageBufferMesh;
...
static int	ble_svc_gatt_handler(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
	...
	case BLE_GATT_ACCESS_OP_WRITE_CHR:
		...
		ESP_LOGI(__FUNCTION__, "ctxt->om->om_len=%d", ctxt->om->om_len);
		ESP_LOGI(__FUNCTION__, "ctxt->om->om_data=[%.*s]", ctxt->om->om_len, ctxt->om->om_data);
		...
		for (int i=0;i<strlen((char *)ctxt->om->om_data);i++) {
			...
			rx_buffer[rx_length] = ctxt->om->om_data[i];
			rx_length++;
		}
		size_t sended = xMessageBufferSendFromISR(xMessageBufferMesh, rx_buffer, rx_length, NULL);
		if (sended != rx_length) {
			ESP_LOGE(__FUNCTION__, "xMessageBufferSendFromISR fail rx_length=%d sended=%d", rx_length, sended);
		}
		break;
...
void nimble_spp_task(void * pvParameters)
{
	...
	char rx_buffer[BUF_SIZE+1];
	char ble_buffer[BUF_SIZE+3];
	char crlf[] = {0x0d, 0x0a};
	while(1){
		size_t received = xMessageBufferReceive(xMessageBufferBle, rx_buffer, BUF_SIZE, portMAX_DELAY);
		ESP_LOGI(pcTaskGetName(NULL), "xMessageBufferReceive received=%d", received);
		//ESP_LOG_BUFFER_HEXDUMP(pcTaskGetName(NULL), rx_buffer, received, ESP_LOG_INFO);
		memcpy(ble_buffer, rx_buffer, received);
		memcpy(ble_buffer+received, crlf, 2);
		//ESP_LOG_BUFFER_HEXDUMP(pcTaskGetName(NULL), ble_buffer, received+2, ESP_LOG_INFO);

		for (int i = 0; i <= CONFIG_BT_NIMBLE_MAX_CONNECTIONS; i++) {
			/* Check if client has subscribed to notifications */
			if (conn_handle_subs[i]) {
				struct os_mbuf *txom;
				//txom = ble_hs_mbuf_from_flat(rx_buffer, received);
				txom = ble_hs_mbuf_from_flat(ble_buffer, received+2);
