...
void tcp_client_write_task(void *arg)
{
	...
	while (1) {
		if (g_sockfd == -1) {
			...
			g_sockfd = socket_tcp_client_create(CONFIG_TCP_SERVER_IP, CONFIG_TCP_SERVER_PORT);
			continue;
		}
		...
		ESP_LOGI(TAG, "TCP write, size=%d ret=%d", size, ret);
		free(data);
...
static void print_system_info_timercb(TimerHandle_t timer)
{
	...
	ESP_LOGD(TAG, "System information, channel: %d, layer: %d, self mac: " MACSTR ", parent bssid: " MACSTR
		", parent rssi: %d, free heap: %"PRIu32"", primary,
		esp_mesh_lite_get_level(), MAC2STR(sta_mac), MAC2STR(ap_info.bssid),
		(ap_info.rssi != 0 ? ap_info.rssi : -120), esp_get_free_heap_size());
#if CONFIG_MESH_LITE_NODE_INFO_REPORT
	ESP_LOGD(TAG, "All node number: %"PRIu32"", esp_mesh_lite_get_mesh_node_number());
#endif /* MESH_LITE_NODE_INFO_REPORT */
	for (int i = 0; i < wifi_sta_list.num; i++) {
		ESP_LOGD(TAG, "Child mac: " MACSTR, MAC2STR(wifi_sta_list.sta[i].mac));
	}
}
